export default class MessageBox {
  ...
  show(msg, label = "CLOSE", callback = null) {
    if ( this.id === null || typeof this.id === "undefined" ) {
      ...
      throw new Error("Please set the 'ID' of the message box container.");
    }

    if ( msg === "" || typeof msg === "undefined" || msg === null ) {
      // If the 'msg' parameter is not set, throw an error

      throw new Error("The 'msg' parameter is empty.");
    }
    ...
    let
      option = this.option,
      msgboxArea = document.querySelector(this.id),
      msgboxBox = document.createElement("DIV"),
      msgboxContent = document.createElement("DIV"),
      msgboxClose = document.createElement("A");

    if ( msgboxArea === null ) {
      // If there is no Message Box container found.

      throw new Error("The Message Box container is not found.");
    }
